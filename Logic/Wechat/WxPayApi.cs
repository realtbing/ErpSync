...
namespace Logic.Wechat
{
    public class WxPayApi
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static WxPayData CodeToSession(WxPayData inputObj, int timeOut = 6)
        {
            WxPayData result = new WxPayData();
            //必要参数校验
            if (!inputObj.IsSet("appid"))
            {
                throw new WxPayException("接口中，appid必填！");
            }
            if (!inputObj.IsSet("secret"))
            {
                throw new WxPayException("接口中，secret必填！");
            }
            if (!inputObj.IsSet("js_code"))
            {
                throw new WxPayException("接口中，js_code必填！");
            }

            string url = WxPayConfig.GetConfig().GetCodeToSessionUrl() + "?appid="+ inputObj .GetValue("appid") + "&secret=" + inputObj.GetValue("secret") + "&js_code=" + inputObj.GetValue("js_code") + "&grant_type=authorization_code";
            
            string _jsonstr = HttpService.Get(url);
            //HttpItem _item = new HttpItem();
            //_item.Method = "GET";
            //_item.URL = url;
            //_item.Timeout = 3000;
            //_item.KeepAlive = false;
            //_item.ProtocolVersion = HttpVersion.Version10;
            //_item.ContentType = "application/json";
            //string _jsonstr = new HttpHelper().Request(_item, "");
            if (!String.IsNullOrEmpty(_jsonstr))
            {
                result.SetValue("DataResult", _jsonstr);
            }
            return result;
        }
    }
